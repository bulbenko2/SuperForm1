...
function setListeners(userCard) {
    ...
    changeBtn.addEventListener('click', () => {
        ...
        document.querySelector('#email').dataset.originalEmail = userEmail
    })
}
...
function addCard(e) {
    ...
    if (!newEmail.value || !newName.value || !newSecondName.value) {
    ...
    const { originalEmail } = newEmail.dataset

    if (originalEmail && originalEmail === newEmail.value) {
        storage[newEmail.value] = data
        localStorage.setItem('users', JSON.stringify(storage))
        rerenderCards(storage)
        console.log(`Обновлен пользователь ${newEmail.value}`)
    } else if (!storage[newEmail.value]) {
        storage[newEmail.value] = data
        ...
        localStorage.setItem('users', JSON.stringify(storage))
        console.log(`Добавлен новый пользователь ${newEmail.value}`)
    } else {
        alert('Пользователь с таким email уже существует!')
    }

    resetInputs(newName, newSecondName, newEmail, newAge, newCity)
    delete newEmail.dataset.originalEmail
}
